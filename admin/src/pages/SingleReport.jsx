...
export default function SingleReport() {
  ...
  const handleResolve = async () => {
    setResolving(true);
    try {
      const response = await resolveReport("resolved", resolutionText, id);
      if (response) {
        setReport((prev) => ({
          ...prev,
          status: "resolved",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Link to={`/business/${report.business._id}`} className="block">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg shadow hover:bg-gray-2 dark:hover:bg-meta-4 transition-shadow">
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="font-medium">{report.business.name}</span>
              </div>
            </div>
          </Link>

          <Link to={`/customer/${report.customer._id}`} className="block">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg shadow hover:bg-gray-2 dark:hover:bg-meta-4 transition-shadow">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
            ...
            <div className="mt-6">
              <h3 className="text-lg font-medium mb-1">Report Date:</h3>
              ...
              {report.status === "resolved" && report.resolvedAt && (
                <div className="mt-2">
                  <h3 className="text-lg font-medium mb-1">Resolved Date:</h3>
                  <p className="text-sm text-muted-foreground">
              ...
              <div className="mt-4 space-y-2">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium">
                    Resolved by {report.resolutionBy.name}
                  </span>
                </div>
                {report.resolution && (
                  <div>

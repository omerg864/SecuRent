...
const BusinessProfileScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchBusinessData = async () => {
      try {
        ...
        console.log("Business data fetched new:", businessData);
      } catch (error) {
  ...
  if (!businessData) {
    router.back(); // Navigate back if no data is found
  ...
  const renderBusinessDetails = () => (
    <ThemedView className="px-4 py-6 bg-white" darkColor="#ffffff">
      {/* Show Edit Profile Button if isBusiness is true */}
    ...
    <View className="flex-row border-b border-gray-300 bg-white">
      <HapticButton
        ...
        className={`flex-1 py-3 ${
          activeTab === "items" ? "border-b-2 border-[#4338CA]" : ""
        }`}
      >
        <ThemedText
          className={`text-center ${
            activeTab === "items" ? "font-bold text-blue-500" : "text-black"
          }`}
          darkColor="black"
        >
          Items
      ...
      <HapticButton
        ...
        className={`flex-1 py-3 ${
          activeTab === "reviews" ? "border-b-2 border-[#4338CA]" : ""
        }`}
      >
        <ThemedText
          className={`text-center ${
            activeTab === "reviews" ? "font-bold text-blue-500" : "text-black"
          }`}
          darkColor="black"
        >
          Reviews
  ...
  const renderReviewsTab = () => (
    <View className="p-4 bg-white">
      <View className="bg-white rounded-lg p-4 mb-6 shadow">
        ...
        <ThemedText className="text-black" darkColor="black">
      ...
      {reviews.map((review: any) => (
        <HapticButton
          ...
          onPress={() => {
            // Handle the button press here (optional)
          }}
        >
          <View className="flex-row items-center mb-3">
            <Image
  ...
  return (
    ...
    >
      <View className="flex flex-col bg-white">
        ...
        {renderTabs()}
        {activeTab === "items" ? renderItemsTab() : renderReviewsTab()}
      </View>
    </ParallaxScrollView>
  );
};

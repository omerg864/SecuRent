...
const BusinessProfileScreen = () => {
	...
	const { business: businessUser } = useBusiness();
	...
	useFocusEffect(
		useCallback(() => {
			...
			fetchBusinessData();
		}, [params.id, params.from, customer, businessUser])
	);
			...
			<ParallaxScrollView
				headerImage={
					business.image ? (
						...
						<View className="items-center justify-center w-full h-full bg-gray-400">
							<Ionicons
					...
					{activeTab === 'items' ? (
						<View className="p-4 bg-white">
							{items.length === 0 ? (
								...
								items.map((item: Item) => (
									<HapticButton
										...
										className="flex-row items-center p-3 mb-6 bg-white rounded-lg shadow"
									>
										<View className="flex-row items-center flex-1">
											{item.image ? (
												...
												<View className="items-center justify-center w-20 h-20 bg-gray-400 rounded-lg">
													<Ionicons
														name="image-outline"
														size={40}
														color="white"
							...
							{(business?.suspended ||
								...
								<View className="bg-gray-100/95 absolute inset-0 z-10 flex items-center justify-center px-6">
									<ThemedText
